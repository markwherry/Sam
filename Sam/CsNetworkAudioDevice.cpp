...
// ------------------------------------------------------------------------------------------------------------------------------------------------------
CsNetworkAudioDevice::CsNetworkAudioDevice(CsAudioManager *audioManager)
...
, m_sendBufferSize(0)
{
	...
	m_deviceInfo.name = AString("Network Audio");
	m_deviceInfo.uuid = AString("F14596A4-3C4F-44C4-BC5D-D37678464740");
	...
	m_udpSocket = new AUdpSocket();
}

// ------------------------------------------------------------------------------------------------------------------------------------------------------
AStatus CsNetworkAudioDevice::init(void)
{
	...
	if(!m_udpSocket->open(9931)) {
		aLog("Error opening socket on port 9931.");
		return AError;
	}
	aLog("Socket open on port 9931.");

	return AOk;
}

// ------------------------------------------------------------------------------------------------------------------------------------------------------
AStatus CsNetworkAudioDevice::exit(void)
{
	return CsAudioDevice::exit();
}

// ------------------------------------------------------------------------------------------------------------------------------------------------------
AStatus CsNetworkAudioDevice::open(AString driverName)
{
	if(m_inputBuffer) {
...
// ------------------------------------------------------------------------------------------------------------------------------------------------------
AStatus CsNetworkAudioDevice::close(void)
{
	return CsAudioDevice::close();
}

// ------------------------------------------------------------------------------------------------------------------------------------------------------
int CsNetworkAudioDevice::start(void)
{
...
// ------------------------------------------------------------------------------------------------------------------------------------------------------
int CsNetworkAudioDevice::stop(void)
{
	CsAudioDevice::stop();
	...
	m_udpSocket->sendBuffer("127.0.0.1", buffer, headerSize);
...
// ------------------------------------------------------------------------------------------------------------------------------------------------------
AString CsNetworkAudioDevice::channelName(int channel, bool isInput)
{
	if(isInput) {
...
// ------------------------------------------------------------------------------------------------------------------------------------------------------
double CsNetworkAudioDevice::sampleRate(void)
{
	return 44100.0;
}

// ------------------------------------------------------------------------------------------------------------------------------------------------------
AStatus CsNetworkAudioDevice::setSampleRate(double sampleRate)
{
...
// ------------------------------------------------------------------------------------------------------------------------------------------------------
void CsNetworkAudioDevice::threadProcess(void)
{
	char buffer[24];
	...
	while(isThreadRunning()) {
		if(m_sendBuffer) {
			...
			m_udpSocket->receiveBuffer(buffer, headerSize, &address);
			//CSDEBUG("Received buffer. address=[%s]", udpSocket->lastReceivedAddress());
			
			if(buffer[0]==100) { // Reset
				aLog("Reset Received from %s.", m_udpSocket->lastReceivedAddress());
			...
			//		memcpy(&m_sendBuffer[24], m_outputBuffer->data(), sizeof(float)*m_bufferSize*m_numOutputs);
					...
					m_udpSocket->sendBuffer(m_udpSocket->lastReceivedAddress(), m_sendBuffer, m_sendBufferSize);
					//CSDEBUG("Sent packet.");
