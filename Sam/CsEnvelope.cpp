...
CsEnvelope::CsEnvelope(double sampleRate, int bufferSize, int samplePeriod)
	: CsModulationSource(sampleRate, bufferSize, samplePeriod, true)
...
// ------------------------------------------------------------------------------------------------------------------------
void CsEnvelope::update(int frameOffset)
{
	//aLog("CsEnvelope::update(%d) %d", frameOffset, bufferSize());
	for(int i=frameOffset; i<bufferSize(); i++) {
		m_buffer[i] = gain();
		incrementSample();
	}
}
...
// ------------------------------------------------------------------------------------------------------------------------
void CsEnvelope::advance(int frames)
{
	//csDebug("CsEnvelope::advnace() gain = %f", gain());
...
void CsEnvelope::setStage(int stageIndex, float timeMs, float gain, int curveShape, float curveDepth)
{
	...
	//aLog("time = %f, timeMs = %f, samplesPerMs = %f", time, timeMs, samplesPerMs);
	...
	if(envelopeStageList[stageIndex]->stageType() == CsEnvelopeStage::Ramp) {
		...
		if(time==0.0) {
			...
			envelopeStageList[stageIndex]->setInitialGain(previousGain);
			envelopeStageList[stageIndex]->setStep((gain-previousGain)/(time/period()));
...
void CsEnvelope::setReleased(void)
{
	...
	//if (released) {	
		...
		envelopeStageList[m_stage]->setStep( (envelopeStageList[m_stage]->gain()-m_gain)/(envelopeStageList[m_stage]->time()/period()) );
		...
		envelopeStageList[m_stage]->setInitialGain(m_gain);
	//}
}
	...
	switch(envelopeStageList[m_stage]->stageType()) {
		...
		case CsEnvelopeStage::Ramp:
			...
			//	int t = (int)(m_time/(float)period());
				...
				float t = (float)m_time/(envelopeStageList[m_stage]->time()-1);
				//float b0 = (1-t)*envelopeStageList[m_stage]->initialGain()+(t*envelopeStageList[m_stage]->gain()); // Linear
				float b0 = (AMath::pow((1-t),2)*envelopeStageList[m_stage]->initialGain()) + (2*(t*(1-t)*0)) + AMath::pow(t,2)*envelopeStageList[m_stage]->gain();
				//b0 = AMath::pow(b0,2);
...
{
	//aLog("CsEnvelope::setNextStage(%d) %f", m_stage+1, m_gain);
	m_time = 0;			
	m_stage++;
	...
	//aLog("m_gain = %f, gain %f", m_gain, envelopeStageList[m_stage]->gain());
	}
...
void CsEnvelopeAhdsr::setEnvelope(float attackTime, double holdTime, double decayTime, float sustainGain, double releaseTime, int attackShape, int decayShape, int releaseShape, float attackDepth, float decayDepth, float releaseDepth, float attackGain)
{
	...
	reset();
	setStage(0, attackTime, attackGain, attackShape, attackDepth);
	setStage(1, holdTime, attackGain);
